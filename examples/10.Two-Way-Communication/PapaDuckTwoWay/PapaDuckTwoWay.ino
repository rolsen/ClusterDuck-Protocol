/**
 * @file PapaDuckTwoWay.ino
 * @author ProjectOwl, Rory Olsen
 * @brief Modification of PapaDuck.ino which broadcasts on an interval
 *
 * @date 2021-06-23
 *
 */
...
// Configs for sending messages to Mama
...
void setup() {
  ...
  timer.every(SEND_MESSAGE_INTERVAL, runSensor);
...
bool runSensor(void *) {
  ...
  Serial.print("[PAPA] sensor data: ");
  Serial.println(message);
  buffer = (byte*) message.c_str(); 

  result = sendData(buffer, length);
  if (result) {
     Serial.println("[PAPA] runSensor ok.");
  } else {
     Serial.println("[PAPA] runSensor failed.");
  }
  return result;
}
...
bool sendData(const byte* buffer, int length) {
  ...
  if (!sentOk) {
    Serial.println("[PAPA] Failed to send data. error = " + String(err));
  }
  return sentOk;
}
